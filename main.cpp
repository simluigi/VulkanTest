...
struct Vertex
{
	glm::vec2 pos;		// glmライブラリーがシェーダーコードに合ってるC++データ型を用意してくれます。
	glm::vec3 color;
	...
	static VkVertexInputBindingDescription getBindingDescription()
	{
		...
		bindingDescription.stride = sizeof(Vertex);						// stride: 次の要素までのバイト数
	...
	static std::array<VkVertexInputAttributeDescription, 2> getAttributeDescriptions()
	{
		...
		attributeDescriptions[0].binding = 0;							// bindingDescriptionと同じ値 same as bindingDescription value
		attributeDescriptions[0].location = 0;							// 頂点シェーダー: (location = 0) in																	
		attributeDescriptions[0].format = VK_FORMAT_R32G32_SFLOAT;		// フォーマットについて：
																		// 普通に以下の組み合わせで宣言されています：
		...
		// attributeDescriptions[1]: Color　（上記とほぼ同じ）
